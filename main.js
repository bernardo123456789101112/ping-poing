...
function gotPoses(results){
	console.log(results);
	if(results.length>0){
		pX=results[0].pose.leftWrist.x;
		pY=results[0].pose.rightWrist.y;
		console.log(pX+" "+" "+pY);
	}
}
...
function start(){
  estado="start";
  document.getElementById("status").innerHTML="O jogo vai começar"
}
function draw(){
  if(estado="start")
...
if(pcscore ==4){
    ...
    text("clique no botão restart",width/2,height/2+30)
    noLoop();
    pcscore = 0;
}
